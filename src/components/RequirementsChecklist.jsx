...
import { CheckCircle2, Circle, ClipboardList, Loader2, Tag } from 'lucide-react';
...
const RequirementsChecklist = ({ businessId, mode = 'view' }) => {
  ...
  const { items = [], submitted, total, hauler_type } = data;
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="w-full bg-gray-100 rounded-full h-1.5">
          <div
            className={`h-1.5 rounded-full transition-all duration-500 ${allDone ? 'bg-emerald-500' : 'bg-amber-400'}`}
            style={{ width: `${pct}%` }}
          />
        </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <div>
                  <span className={`text-sm transition-colors ${
                    item.is_submitted
                      ? 'text-gray-700 line-through decoration-emerald-400'
