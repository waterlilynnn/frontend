...
const InspStatusBadge = ({ status, isResolved }) => {
  ...
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700">
      <AlertTriangle className="h-3 w-3" />With Violation
    </span>
  );
};
...
const Reports = ({ rolePrefix = 'staff' }) => {
  ...
  const inRange = (dateStr) => {
    ...
    return d >= new Date(dateFrom) && d <= endOfDay(new Date(dateTo));
  };
  ...
  const filteredInspections = useMemo(() => {
    const base = allInspections.filter(i => {
      ...
      if (search) {
        const q = search.toLowerCase();
        if (!i.establishment_name?.toLowerCase().includes(q) && !i.bin_number?.toLowerCase().includes(q)) return false;
      }
      return true;
    });
    ...
    if (filterInspStatus === 'all') {
      const unresolved = base
        .filter(i => i.status === 'WITH VIOLATION' && !i.is_resolved)
      ...
      <div className="space-y-5">
        <div className="flex flex-wrap gap-3 items-center justify-between">
          ...
          <button onClick={handleExportPDF} disabled={pdfLoading}
            className="inline-flex items-center px-3 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm disabled:opacity-60 transition-colors">
            {pdfLoading
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input type="text"
                    placeholder={tab === 'clearances' ? 'Search business, control #…' : 'Search business name, BIN…'}
                    value={search}
                ...
                <span className="ml-auto text-xs text-gray-400 font-medium whitespace-nowrap">
                  {active.length} record{active.length !== 1 ? 's' : ''}
                </span>
              </div>
            </div>
                      ...
                      {paginated.map(c => (
                        <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <div className="text-sm font-medium text-gray-900">{c.business_name || '—'}</div>
                            <div className="text-xs text-gray-400 font-mono">BIN: {c.bin_number || '—'}</div>
                          </td>
                          <td className="px-5 py-3 text-sm text-gray-600">{c.hauler_type || '—'}</td>
                          <td className="px-5 py-3 text-sm text-gray-500 whitespace-nowrap">
                            {c.last_printed_at ? fmtDt(c.last_printed_at) : fmtDt(c.printed_at)}
                          </td>
                          <td className="px-5 py-3">
                            {c.is_claimed
                      ...
                      {paginated.map(i => {
                        ...
                        return (
                          <tr key={i.id} className={`transition-colors ${
                            ...
                            <td className="px-5 py-3">
                              <div className="text-sm font-medium text-gray-900">{i.establishment_name}</div>
                              <div className="text-xs text-gray-400 font-mono">BIN: {i.bin_number || '—'}</div>
                            </td>
                            ...
                            <td className="px-5 py-3 max-w-[280px]">
                              {i.remarks && (
                                <p className="text-sm text-gray-700 break-words">{i.remarks}</p>
                              )}
                              {isResolved && (
                                <div className="mt-1.5 pt-1.5 border-t border-blue-100">
                                  ...
                                  {i.resolved_remarks && (
                                    <p className="text-xs text-blue-700 mt-0.5 break-words">{i.resolved_remarks}</p>
                                  )}
                                </div>
                              )}
                              {!i.remarks && !isResolved && (
