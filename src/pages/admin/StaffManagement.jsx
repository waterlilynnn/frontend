    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-2">
                Are you sure you want to deactivate <strong>{showDeactivateConfirm.full_name}</strong>?
              </p>
              <p className="text-sm text-gray-500 mb-6">
                This staff member will no longer be able to access the system until reactivated.
              </p>
              <div className="flex justify-end gap-3">
                <button onClick={() => setShowDeactivateConfirm(null)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                  Cancel
                </button>
                <button onClick={confirmDeactivate} disabled={toggleStatus.isLoading} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                  {toggleStatus.isLoading ? 'Processing...' : 'Deactivate'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4 text-emerald-600">
                ...
                <h3 className="text-lg font-bold text-gray-900">Activate Staff Account</h3>
              </div>
              <p className="text-gray-600 mb-2">
                Are you sure you want to activate <strong>{showActivateConfirm.full_name}</strong>?
              </p>
              {activeStaff.length >= MAX_STAFF && (
                <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-sm text-amber-700 flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4" />
                    Maximum of {MAX_STAFF} active staff accounts reached. Please deactivate another staff member first.
                  </p>
                </div>
              )}
      ...
      <div className="pb-6 lg:pb-28 space-y-4">
        <div className="flex justify-between items-center gap-3">
          ...
          <button onClick={handleNewStaffClick} className="inline-flex items-center px-3 sm:px-4 py-2 bg-emerald-700 text-white rounded-lg hover:bg-emerald-800 shadow-sm text-sm">
                        ...
                        <td className="px-6 py-4">
                          {member.is_active ? (
                            <button
                              ...
                              className="px-3 py-1 border border-red-600 text-red-600 text-xs rounded hover:bg-red-50 disabled:opacity-50"
                            >
                              Deactivate
                            </button>
                          ) : (
                            <button
                              ...
                              disabled={toggleStatus.isLoading}
                              className="px-3 py-1 border border-emerald-700 text-emerald-700 text-xs rounded hover:bg-forest-50 disabled:opacity-50"
                            >
                              Activate
                            </button>
                          )}
              ...
              <div className="md:hidden divide-y divide-gray-100">
                {items.map((member) => (
                  <div key={member.id} className="p-4 flex items-center justify-between gap-3">
                    ...
                    {member.is_active ? (
                      <button onClick={() => handleDeactivateClick(member)} className="shrink-0 px-3 py-1 border border-red-600 text-red-600 text-xs rounded hover:bg-red-50">
                        Deactivate
                      </button>
                    ) : (
                      <button onClick={() => handleActivateClick(member)} className="shrink-0 px-3 py-1 border border-emerald-700 text-emerald-700 text-xs rounded hover:bg-forest-50">
                        Activate
                      </button>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <button onClick={() => { setShowLimitModal(false); setSelectedToDeactivate(null); }} className="text-gray-400 hover:text-gray-600">
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto mb-5">
                {activeStaff.map(s => (
                  <label key={s.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                    selectedToDeactivate === s.id ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                  }`}>
                    <input type="radio" name="deactivate-staff" value={s.id} checked={selectedToDeactivate === s.id} onChange={() => setSelectedToDeactivate(s.id)} className="h-4 w-4 text-red-600" />
                    <div className="flex-1 min-w-0">
              ...
              <div className="flex justify-end gap-3">
                <button onClick={() => { setShowLimitModal(false); setSelectedToDeactivate(null); }} className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">
                  Cancel
                </button>
                <button onClick={handleDeactivateAndProceed} disabled={!selectedToDeactivate || toggleStatus.isLoading} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700 disabled:opacity-50">
                  Deactivate
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setShowCreateModal(false); setNewStaff({ full_name: '', email: '' }); }} className="text-gray-400 hover:text-gray-600">
                  <X className="h-6 w-6" />
                </button>
              </div>
              <form onSubmit={handleCreateStaff} className="space-y-4">
                <div>
                  ...
                  <input type="text" value={newStaff.full_name} onChange={(e) => setNewStaff({ ...newStaff, full_name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 text-sm" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Email Address <span className="text-red-500">*</span></label>
                  <input type="email" value={newStaff.email} onChange={(e) => setNewStaff({ ...newStaff, email: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 text-sm" required />
                </div>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-sm font-medium text-blue-800">Auto-generated password</p>
                  <p className="text-xs text-blue-600 mt-1">A random temporary password will be shown after creation.</p>
                </div>
                <div className="flex justify-end gap-3 pt-2 border-t border-gray-200">
                  <button type="button" onClick={() => { setShowCreateModal(false); setNewStaff({ full_name: '', email: '' }); }} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 text-sm">Cancel</button>
                  <button type="submit" disabled={creating} className="px-4 py-2 bg-emerald-700 text-white rounded-lg hover:bg-emerald-800 disabled:opacity-50 flex items-center gap-2 text-sm">
