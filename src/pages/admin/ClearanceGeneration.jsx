...
const AdminClearance = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      // Get all businesses (all businesses are approved)
      let businessesRes;
      ...
      const items = approvedBusinesses.map(business => {
        ...
        return {
          ...
          print_count: clearance?.print_count || 0,
  ...
  const handleView = (clearanceId) => {
    navigate(`/admin/clearance/${clearanceId}/view`);
  };
                    ...
                    {items.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            by: {item.last_printed_by || item.printed_by || '—'} • {item.print_count} print(s)
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {item.has_clearance ? (
                            <div className="flex items-center space-x-3">
                              <button
                                onClick={() => handleView(item.clearance_id)}
                            ...
                            <button
                              ...
                              className="inline-flex items-center px-3 py-1 bg-emerald-600 text-white text-xs rounded hover:bg-emerald-700 transition-colors"
                            >
                              Generate
                            </button>
                          )}
